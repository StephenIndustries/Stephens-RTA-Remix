...
#define LED_YELLOW 50, 50, 0
#define LED_GREEN 0, 50, 0
...
int gridtostrip(int x, int y);
void fillRect(int x_origin, int y_origin, int x_length, int y_length, int r, int g, int b);
...
void setup() {
  FastLED.addLeds<WS2812, LED_PIN, GRB>(matrix, NUM_LEDS);
  matrix[0].b = 50;
  FastLED.show();
  Serial.begin(9600);
...
void loop() {
  ...
  for(x=0; x<8; x++) {
    ...
    y = 8 - peak[x];
    if(y < 2)      matrix[gridtostrip(x, y)].setRGB(LED_RED);
    else if(y < 6) matrix[gridtostrip(x, y)].setRGB(LED_YELLOW);
    else           matrix[gridtostrip(x, y)].setRGB(LED_GREEN);
  }
...
void drawLine2(int x1, int y1, int x2, int y2, int r, int g, int b) {
  ...
  if (x1 < x2 || y1 < y2) { 
    for (i=0; i < y2; i++) {
      ...
      matrix[gridtostrip(x1, y1 - i)].setRGB(r, g, b);
    
    }
    for (i=0; i < x2; i++) {
      matrix[gridtostrip(x1 - i, y1)].setRGB(r,g,b);
    }
  }
  else {
    for (i=0; i < y2; i++) {
